
/*
 * File: GildedRose.cs
 * --------------------
 * This file contains the GildedRose class. It takes a list of items and updates the quality.
 */

using System.Collections.Generic;

namespace GildedRose
{
	public class GildedRose
	{
		private IList<Item> Items;

		public GildedRose(IList<Item> Items) 
		{
			this.Items = Items;
		}
		
		public void UpdateQuality()
		{
			for (var i = 0; i < Items.Count; i++)
			{
                UpdateQualityForItem(i);
			}
		}

        private void UpdateQualityForItem(int i)
        {
            if (!ItemNameIsAgedBrie(i) && !ItemNameIsBackstagePassForConcert(i))
            {
                if (Items[i].Quality > 0)
                {
                    if (Items[i].Name != "Sulfuras, Hand of Ragnaros")
                    {
                        Items[i].Quality = Items[i].Quality - 1;
                    }
                    else
                    {
                        Items[i].Quality = 80;
                    }

                    if (Items[i].Name == "Conjured")
                    {
                        Items[i].Quality = Items[i].Quality - 1;
                    }
                }
            }
            else
            {
                if (Items[i].Quality < 50)
                {
                    Items[i].Quality = Items[i].Quality + 1;

                    if (ItemNameIsAgedBrie(i) || ItemNameIsBackstagePassForConcert(i))
                    {
                        if (Items[i].SellIn < 11)
                        {
                            if (Items[i].Quality < 50)
                            {
                                Items[i].Quality = Items[i].Quality + 1;
                            }
                        }

                        if (Items[i].SellIn < 6)
                        {
                            if (Items[i].Quality < 50)
                            {
                                Items[i].Quality = Items[i].Quality + 1;
                            }
                        }
                    }
                }
            }

            if (Items[i].Name != "Sulfuras, Hand of Ragnaros")
            {
                Items[i].SellIn = Items[i].SellIn - 1;
            }

            if (Items[i].SellIn < 0)
            {
                if (!ItemNameIsAgedBrie(i))
                {
                    if (!ItemNameIsBackstagePassForConcert(i))
                    {
                        if (Items[i].Quality > 0)
                        {
                            if (Items[i].Name != "Sulfuras, Hand of Ragnaros")
                            {
                                Items[i].Quality = Items[i].Quality - 1;
                            }
                        }
                    }
                }

                if (ItemNameIsAgedBrie(i) || ItemNameIsBackstagePassForConcert(i))
                {
                    Items[i].Quality = 0;
                }
            }
        }

        private bool ItemNameIsAgedBrie(int i)
        {
            return (Items[i].Name == "Aged Brie");
        }

        private bool ItemNameIsBackstagePassForConcert(int i)
        {
            return (Items[i].Name == "Backstage passes to a TAFKAL80ETC concert");
        }
		
	}
	
}